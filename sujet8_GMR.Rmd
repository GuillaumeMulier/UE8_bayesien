...
Sur le graphique d'auto-corrélations, on voit que le problème a été réglé et que maintenant il n'y a plus d'auto-corrélation pour le paramètre a estimé. De plus, le nombre d'itérations effectives est maintenant de `r modele1_fit_thin$BUGSoutput$n.sims` itérations : `r round(ess_1_thin["a"])` pour l'estimation de a et `r round(ess_1_thin["deviance"])` pour l'estimation de la déviance du modèle.
...
Pour les 3 chaînes, on ne voit pas d'auto-corrélation dans notre modèle. Aussi, le nombre d'itérations effectives est resté autour des `r modele1_fit_mult$BUGSoutput$n.sims` itérations faites : `r round(ess_1_mult["a"])` pour l'estimation de a et `r round(ess_1_mult["deviance"])` pour l'estimation de la déviance du modèle (à noter que l'effective sample size égale ici le nombre d'itérations réellement faites. Cela est peut-être du à l'indépendantes des itérations les unes des autres).
...
```{r, warning = FALSE, fig.height = 8, fig.with = 7}
poisson_dens <- tibble(
  x = 0:60,
...
Les résultats du modèle apparaîssent très similaires, avec un a moyen et son intervalle de crédibilité à 95% de `r ic_sens`.
...
On voit que pour l'estimation de $a_0$ et $b_0$, il y a auto-corrélation jusqu'à la 15ème mesure. Pour la déviance, en revanche, cela va jusquà 10.  De plus, le nombre d'itérations effectives est bien en-dessous du nombre d'itérations réalisées : nous avons fait `r modele2_fit$BUGSoutput$n.sims` itérations, et le nombre d'itérations effectives n'est que de `r round(ess_2["a0"])` pour $a_0$ et de `r round(ess_2["b0"])` pour $b_0$. Ce nombre diminué indique une grande auto-corrélation des valeurs estimées entre les itérations.
...
ggplot() +
  ...
  annotate("text", x = 3, y = 125, label = "Moyenne des lois de Poisson 'moyennes'", hjust = 0) +
...
Les moyennes des lois de Poisson avec le $\lambda_i$ estimé estiment bien les données dont nous disposons. Toutefois, pour construire cette figure, nous avons émis l'hypothèse que $a_0$ et $b_0$ varient selon leur intervalle de crédibilité de façon simultanée et identique, ce qui n'est pas forcément vrai.
...
**Selon le modèle que vous avez retenu, dire quelles machines (s'il y en a) ont une moyenne du nombre de pannes probablement (c'est à dire à 97,5%) supérieure à 40 pannes (valeur pour laquelle la machine sera remplacée) ?**
...
Sur le plot des auto-corrélations, nous remarquons que nos itérations sont ben indépendantes les unes des autres. Cela est corroboré par les effective sample sizes pour $a_0$, $b_0$, la déviance et $\tau$ (`r paste(round(ess_3_thin_ch1), collapse = "/")`) qui sont proches des `r round(modele3_fit_thin_ch1$BUGSoutput$n.sims)` itérations effectuées.

Pour nous assurer de la bonne convergence de nos paramètres, nous avons tenté plusieurs initiations des 3 paramètres. Nous avons initié $a_0$ et $b_0$ à -5, 0 et 5, et $\tau$ à 0.05, 0.5, 1 et 2.
...
```{r, echo = FALSE}
include_graphics("plot_abt_36c.png")
```
...
# gelman3 <- ggs_Rhat(gg_modele3_thin %>% mutate(Parameter = case_when(Parameter == "a0" ~ "a<sub>0</sub>",
#                                                    Parameter == "b0" ~ "b<sub>0</sub>",
#                                                    Parameter == "tau" ~ "&tau;",
#                                                    TRUE ~ as.character(Parameter)))) +
#   labs(y = "Paramètre",
#        x = expression(hat("R"))) +
#   theme(axis.text.y = element_markdown())
# geweke3 <- ggs_geweke(gg_modele3_thin%>% mutate(Parameter = case_when(Parameter == "a0" ~ "a<sub>0</sub>",
#                                                    Parameter == "b0" ~ "b<sub>0</sub>",
#                                                    TRUE ~ as.character(Parameter)))) +
#   labs(y = "Paramètre",
#        title = "Critère de convergence de Geweke") +
...
```{r, echo = FALSE}
include_graphics("plot_gelman_geweke.png")
```
...
```{r, echo = FALSE}
include_graphics("plot_autocor.png")
```

Le graphique des auto-corrélations nous montre quant à lui qu'il n'y a pas d'auto-corrélation résiduelle sur nos chaînes.

Les résultats pour les 2 modèles (1 chaîne vs 36 chaînes) sont présentés dans le tableau suivant. On peut remarquer que les résultats sont presque identiques, ce qui nous rassure pour prendre le résultat du modèle à 1 chaîne comme résultat principal. Par ailleur, on remarque que l'écart-type de $\tau$ est très élevé malgré le grand nombre d'itérations. Cela peut nous indiquer que la variable $\varepsilon_i$ de chaque machine suivant une loi normale de même variance pour toutes les machines n'est pas forcément pertinente dans notre modèle. La variabilité n'est peut-être pas tout à fait identique entre chaque machine et cela pourrait empêcher l'estimation d'être précise.
...
**Que vaut le nombre d’itérations pour les calculs? Que vaut le nombre d’itérations
"effectif" ?**

Le nombre d'itérations faites et conservées est de `r modele3_fit_thin_ch1$BUGSoutput$n.sims`. 

Les nombres d'itérations effectives pour les différents paramètres estimés sont :

* pour $a_0$ `r round(ess_3_thin_ch1["a0"])`;
* pour $b_0$ `r round(ess_3_thin_ch1["b0"])`;
* pour $\tau$ `r round(ess_3_thin_ch1["tau"])`;
* pour la déviance `r round(ess_3_thin_ch1["deviance"])`.

Ces nombres effectifs proches du nombre d'itérations gardées montrent l'indépendance de nos itérations.

## Question 4

**Donnez la moyenne a posteriori et l'intervalle de crédibilité à 95% de a0, b0 et tau.**

```{r}
resm3 <- summary(modele3_fit_thin_ch1_mcmc)[[1]] %>% as.data.frame()
resq3 <- summary(modele3_fit_thin_ch1_mcmc)[[2]] %>% as.data.frame()
ic3a <- paste0(round(resm3[["Mean"]][1], 2), "[", round(resq3[["2.5%"]][1], 2), ";", round(resq3[["97.5%"]][1], 2),"]")
ic3b0 <- paste0(round(resm3[["Mean"]][2], 2), "[", round(resq3[["2.5%"]][2], 2), ";", round(resq3[["97.5%"]][2], 2),"]")
ic3tau <- paste0(round(resm3[["Mean"]][4], 2), "[", round(resq3[["2.5%"]][4], 2), ";", round(resq3[["97.5%"]][4], 2),"]")
ic3sig <- paste0(round(1/resm3[["Mean"]][4], 2), "[", round(1/resq3[["97.5%"]][4], 2), ";", round(1/resq3[["2.5%"]][4], 2),"]")
```

L'estimation de $a_0$ nous donne la moyenne et l'intervalle de crédibilité à 95% suivant : `r paste(ic3a)`.
L'estimation de $b_0$ nous donne la moyenne et l'intervalle de crédibilité à 95% suivant : `r paste(ic3b0)`.
L'estimation de $\tau$ nous donne la moyenne et l'intervalle de crédibilité à 95% suivant : `r paste(ic3tau)`, soit un intervalle de crédibilité pour $\sigma^2$ de `r paste(ic3sig)`.

Nous avons tenté de représenter ces moyennes de lois de Poisson sur les données dont nous disposons. Les lignes "moyenne", "minimum" et "maximum" représentent les moyennes des lois de Poisson lorsque $a_0$ et $b_0$ sont  respectivement toutes les 2 à leur valeur moyenne, à la borne inférieure ou supérieure de leur intervalle de crédibilité à 95%. De plus, autour de ces estimations de moyenne, nous avons représenté la variabilité inter-machine $\varepsilon_i$ qui est assuméee comme l'intervalle de confiance à 95% de la loi normale suivie par $\varepsilon_i (\mathcal{N}(0, \sigma^2))$ pour le $\sigma^2$ moyen estimé.

```{r, fig.width = 10, fig.height = 8}
poissons_moy <- tibble(
  anciennete = 0:15,
  poiss_moymin = exp(0.08 + anciennete * 0.14),
  poiss_moymininf = exp(0.08 + anciennete * 0.14 + qnorm(0.025, mean = 0, sd = 1/15)),
  poiss_moyminsup = exp(0.08 + anciennete * 0.14 + qnorm(0.975, mean = 0, sd = 1/15)),
  poiss_moymean = exp(0.86 + anciennete * 0.21),
  poiss_moymeaninf = exp(0.86 + anciennete * 0.21 + qnorm(0.025, mean = 0, sd = 1/15)),
  poiss_moymeansup = exp(0.86 + anciennete * 0.21 + qnorm(0.975, mean = 0, sd = 1/15)),
  poiss_moymax = exp(1.53 + anciennete * 0.28),
  poiss_moymaxinf = exp(1.53 + anciennete * 0.28 + qnorm(0.025, mean = 0, sd = 1/15)),
  poiss_moymaxsup = exp(1.53 + anciennete * 0.28 + qnorm(0.975, mean = 0, sd = 1/15))
)
ggplot() +
  geom_point(data = sncf_machines, aes(anciennete, nb_pannes)) +
  geom_ribbon(data = poissons_moy, aes(x = anciennete, ymin = poiss_moymeaninf, ymax = poiss_moymeansup), fill = "green", alpha = 0.3) +
  geom_line(data = poissons_moy, aes(x = anciennete, y = poiss_moymean, color = "Moyenne")) +
  geom_ribbon(data = poissons_moy, aes(x = anciennete, ymin = poiss_moymininf, ymax = poiss_moyminsup), fill = "blue", alpha = 0.3) +
  geom_line(data = poissons_moy, aes(x = anciennete, y = poiss_moymin, color = "Inférieur"), linetype = "dashed") +
  geom_ribbon(data = poissons_moy, aes(x = anciennete, ymin = poiss_moymaxinf, ymax = poiss_moymaxsup), fill = "red", alpha = 0.3) +
  geom_line(data = poissons_moy, aes(x = anciennete, y = poiss_moymax, color = "Supérieur"), linetype = "dashed") +
  scale_color_manual(name = NULL,
...
Nous pouvons voir sur la réprésentation que le modèle englobe maintenant toutes les machines. Cependant, on remarque aussi que l'incertitude devient plus grande pour les bornes. La droite des moyennes des lois de Poissons "moyennes" a l'air de bien représenter nos données, mais la borne haute s'envole beaucoup plus haut que le dernier modèele à cause du coefficient $b_0$ plus haut.

De plus, tout comme le modèle 2, nous avons fait l'hypothèse que $a_0$ et $b_0$ suivent leur intervalle de crédibilité à 95% de façon parallèle, et nous n'avons pris le $\tau$ que moyen.

## Question 5

**Que vaut le DIC ? Que vaut l'estimation de la complexité du modèle ? Vous semble-t-elle
logique ?**
...
Le DIC du modèle vaut `r dic3`.

Pour estimer la complexité du modèle, le pD est estimé et représente le nombre effectif de paramètres estimés. Pour notre modèle, il vaut `r complexite3`. 

Nous avions 3 paramètres à estimer : $a_0$, $b_0$ et $\tau$, donc nous nous attendions à avoir environ un pD de 3. Mais comme évoqué plus tôt, le paramètre $\tau$ a un écart-type élevé et nous avions émis l'hypothèse que les machines n'avaient peut-être pas toutes la même variabilité inter-individuelle. Ici, on a l'impression que cela va dans le sens de cette hypothèse, avec un nombre effectif de paramètres estimés environ augmenté de 10 par rapport à ce que nous pensions trouver sur un échantillon de 10 machines. Nous pensons donc que le modèle spécifié n'est pas le plus adapté aux données.

## Question 6

**D'après le DIC, quel modèle choisissez-vous entre M1, M2 et M3 ?**
