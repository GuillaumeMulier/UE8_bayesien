...
Le nombre d'itérations pour notre calcul est de 240000 (30 000 * 8) mais nous avons mis un thin de _10_, le nombre d'itéartions effectif est de 29875. Cela représente le nombre d'itérations qui ont apportées de l'information utile à notre modèle.

```{r}
effectiveSize(modele1_fit_thin)
```

## Question 4 : 
Donnez la moyenne a posteriori et l'intervalle de crédibilité à 95% de a. 

La moyenne à posteriori du *a* est de 3.00 avec un intervalle de crédibilité à 95% à [2.86;3.13]
````{r, warning = FALSE, message = FALSE, result = 'hide'}

res1 = summary(modele1_fit_thin_mcmc)
resmoy1 = as.data.frame(res1[[1]])
resq1 = as.data.frame(res1[[2]])


a1_2.5 = resq1$'2.5%'[1]
a1_97.5 = resq1$'97.5%'[1]
...
Le DIC vaut 253 et l'estimation de la complexité du modèle 1 (pD). cela est cohérent puisque nous n'avions qu'un seul paramètre *a*.

```{r, warning = FALSE, message = FALSE, result = 'hide'}
print(modele1_fit_thin)
...
Nous avons donc réalisé le même modèle mais avec une loi à priori sur a de variance 10 000 contre 1 000 initialement. Ce procédé nous permet d'utiliser une loi encore moins infiormative et de voir si cette dernière a eu un impact sur le calcul de nos premirers résultats.

Nous avons donc réitéré ce modèle, vérifié que la convergence était atteinte et qu'il n'y avait pas d'autocorrélation (nous avons gardé comme paramètre un thin à 8 et par conséquent un nombre d'itération à 30 000 * 8 pour garder environ 30 0000 intértions effectives).

Les résultats sont semblable entre les deux modèles. Tout d'abord le DIC et l'estimation de a complexité du modèle sont identique : respectivement 253 et 1.

La moyenne de *a* est quasiment identique à  2.998186 contre 2.997857 pour le modèle initial et il en est de même pour les bornes de l'intervalle de crédibilité à 95% [2.857767;3.135289] pour notre nouveau modèle contre [2.858005;3.133286] pour le modèle précédent.

La modification de la loi à priori n'a quasiment pas modifié les résultats obtenus. Nous pouvons donc considéré que la première loi à une variance suffisante pour être considérée comme non informative.

```{r, warning = FALSE, message = FALSE, result = 'hide'}
modele_1bis <- function() {
  # Modèle pour yi
  for (i in 1:10) {
    nb_pannes[i] ~ dpois(exp(a))
  }
...
modele1bis_fit <- jags(data = donnees,
                    inits = inits_modele1,
                    parameters.to.save = parametres_modele1, 
                    n.chains = length(inits_modele1),
                    n.iter = n_iter*8, 
                    ...
                    n.thin = 8,
                    model.file = modele_1) 
modele1bis_fit_mcmc <- as.mcmc(modele1bis_fit)

# confirmation convergence et de l'absence d'autorcoriélation
gg_modele1bis <- ggs(modele1bis_fit_mcmc)
ggplot(gg_modele1bis %>% filter(Parameter == "a"), aes(x = Iteration, y = value)) +
  geom_line() +
  scale_x_continuous(labels = scales::comma_format()) +
  labs(x = "Itération",
       y = "a",
       title = "Traceplot de l'estimation de a par MCMC",
       subtitle = "Initialisation de a à 1000 ; Prise d'une valeur sur 8")
ggs_autocorrelation(gg_modele1bis)

# Même BIC et même complexité
print(modele1bis_fit)

# Moyenne et IC.95% identiques
res1bis = summary(modele1bis_fit_mcmc)
resmoy1bis = as.data.frame(res1bis[[1]])
resq1bis = as.data.frame(res1bis[[2]])


a1bis_2.5 = resq1bis$'2.5%'[1]
a1bis_97.5 = resq1bis$'97.5%'[1]
```
...
exp(b0), qui vaut 1.2, représente le sur-risque par année passée de développer une panne. En d'autres termes, chaque année, le nombre risque de développer une panne augemente de 1.2.

## Question 7 :
Que vaut le DIC ? Que vaut l'estimation de la complexité du modèle ? Vous semble-t-elle logique ?

Le DIC du modèle vaut 69.4 et l'estimation de la complexité du modèle (pD) vaut 2.0. Cela est cohérent puisque nous avions 2 paramètres dans notre modèle (a et b0).  
```{r, warning = FALSE, message = FALSE, result = 'hide'}
print(modele2_fit_thin)
print(modele1_fit_thin)
...
En relevant les DIC de nos deux modèles, nous choissons le modèle M2. Ce dernier à un DIC nettement inférieur à celui du modèle 1, 69.4 contre  253. L'apporte de la variable x(temps) est donc associé une meilleur estimation du modèle.